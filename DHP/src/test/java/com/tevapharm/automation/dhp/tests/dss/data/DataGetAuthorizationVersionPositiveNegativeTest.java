...
public class DataGetAuthorizationVersionPositiveNegativeTest extends PartnerApiTestBase {
    ...
    public void tc01_getPartnerLocalAuthorizationsPositive() throws IOException {
        ...
        TevaAssert.assertEquals(extentTest, js2.get("authorizations.hipaaDisclosure"), PropertyUtils.readProperty("hipaaText"), "Hipaa Disclosure content is correct");
        TevaAssert.assertEquals(extentTest, js2.get("authorizations.signaturePolicy"), PropertyUtils.readProperty("signatureText"), "Signature Policy content is correct");
        TevaAssert.assertEquals(extentTest, js2.get("authorizations.marketingConsent"), PropertyUtils.readProperty("marketingText"), "Marketing Consent content is correct");
        TevaAssert.assertEquals(extentTest, js2.get("authorizations.termsOfUse"), PropertyUtils.readProperty("termsOfUseText"), "Terms Of Use content is correct");
        TevaAssert.assertEquals(extentTest, js2.get("authorizations.privacyNotice"), PropertyUtils.readProperty("privacyNoticeText"), "Privacy Notice content is correct");

    }

    @Test(priority = 1, testName = "Get partner local authorizations by patient for partner with no legal consent - negative test")
    @Traceability(FS = {"1662"})
    public void tc02_getLocalAuthorizationNoLegalNegative() throws IOException {
        ExtentTest extentTest = ExtentManager.getTest(this.getClass());
        ...
        Response responseFileE = given()
                ...
                .multiPart("file", signatureTxt, "text/plain")
                .log().all()
                .post(PropertyUtils.readProperty("onboardSignature02ConsentUrl"))
                .then()
                .log().all()
                .extract().response();
        ...
        extentTest.info("Get the latest version of authorization language for this partner by the user, Expecting HTTP Response error code '404', because some partner legal consent not found");
        Response resGetData = given().log().all()
                .baseUri(PropertyUtils.readProperty("platformUrl"))
                .filter(new ConsoleReportFilter(extentTest))
                .header("Authorization", "Bearer " + accessToken)
    ...
    @Traceability(FS = {"1662"})
    public void tc03_getLocalAuthorizationDeletedPartnerNegative() throws IOException, InterruptedException {
        ExtentTest extentTest = ExtentManager.getTest(this.getClass());
